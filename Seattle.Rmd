...
#let's put each destination in a different day
locs.sites$time <- as.Date(locs.sites$time)+1:length(locs.sites$time)
View(locs.sites)
...
daily_routes <- function(date) {
  ll <- locs.sites %>% filter(as.Date(time)==date) %>%
    select(lon,lat)
  #add home early in the morning 	
  ll <- rbind(data.frame(lon = -122.3610036, lat = 47.6646144), ll)
  #add home late in the day
  ll <- rbind(ll, data.frame(lon = -122.3610036, lat = 47.6646144))
  route <- NULL
  for (j in 2:nrow(ll)) {
